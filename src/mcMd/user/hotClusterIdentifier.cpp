#ifndef HOT_CLUSTER_IDENTIFIER_CPP
#define HOT_CLUSTER_IDENTIFIER_CPP
...
#include "hotClusterIdentifier.h"
...
namespace McMd
{
   ...
   * Constructor.
   */
   hotClusterIdentifier::hotClusterIdentifier(System& system)
    : links_(),
      clusters_(),
      workStack_(),
      cellList_(),
   ...
   * Destructor.
   */
   hotClusterIdentifier::~hotClusterIdentifier()
   {}
   ...
   */
   void
   hotClusterIdentifier::initialize(int speciesId, int atomTypeId, double cutoff)
   {
   ...
   */
   void hotClusterIdentifier::processNextMolecule(Cluster& cluster, DArray <bool> isHot)
   {
      ...
      */
      ...
      // one check for hot bead should come here    
         if (isHot [atomIter->id()] == 1) { 
            if (atomIter->typeId() == atomTypeId_) {
               ...
               for (int i = 0; i < neighborArray.size(); i++) {
                  ...
                  // second check for hot bead should come here
                  if (isHot [otherAtomPtr->id()] == 1) {
   ...
   */
   void hotClusterIdentifier::identifyClusters(DArray <bool> isHot)
   {
      ...
      for ( ; molIter.notEnd(); ++molIter) {
         ...
         if (linkPtr->clusterId() == -1) {
            ...
            while (workStack_.size() > 0) {
               processNextMolecule(*clusterPtr, isHot);
            }
   ...
   bool hotClusterIdentifier::isValid() const
   {
      // Check clusters
